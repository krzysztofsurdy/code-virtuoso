...
description: Behavioral pattern that allows objects to change behavior when their internal state changes. The object appears to change its class. Use when an object's behavior depends on state and must change at runtime based on that state.
---
